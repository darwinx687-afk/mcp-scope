...
- v0.3 工作中已经支持从源码生成 SARIF，但 GitHub Code Scanning 上传仍然需要 workflow 显式执行。
- 还没有覆盖所有 MCP 客户端。
- 工具元数据需要本地导出或本地提供。
...
- `scan`、`inspect-tools` 和 `audit` 的 SARIF 输出。
- `audit --root <path>` 一条命令组合静态 discovery 和静态 config scan 摘要。
- 可选的 GitHub Code Scanning SARIF 上传示例。
- 静态-only 边界不变。
